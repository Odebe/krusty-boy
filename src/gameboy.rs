...
impl Emulator {
    ...
    pub fn load_rom(&mut self, rom: &Vec<u8>) {
        let ustart = Self::PROG_START as usize;
        for (i, e) in rom.iter().enumerate() {
            if ustart + i < Self::MEMORY_SIZE as usize { self.memory[ustart + i] = *e; }
        }
    }
    ...
    fn read_u16(&self, from: u16) -> u16 {
        let value_fn = self.memory[from as usize];
        let value_sn = self.memory[(from + 1) as usize];
        ((value_sn as u16 ) << 8) | value_fn as u16
    }

    fn inc_pc_by(&mut self, bytes: u8) {
